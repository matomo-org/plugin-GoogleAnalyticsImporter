...
trait LoggingTrait
{
    ...
    private function logRequest(RpcLogEvent $event) : void
    {
        ...
        $debugEvent = array_filter($debugEvent, function ($value) {
        ...
        // Remove null values
        $debugEvent['jsonPayload'] = array_filter($jsonPayload, function ($value) {
            return !is_null($value);
        });
    ...
    private function logResponse(RpcLogEvent $event) : void
    {
        ...
        $debugEvent['jsonPayload'] = array_filter($debugEvent['jsonPayload'], function ($value) {
            return !is_null($value);
        });
        $stringifiedEvent = json_encode($debugEvent, \JSON_UNESCAPED_SLASHES | \JSON_UNESCAPED_UNICODE);
        // There was an error stringifying the event, return to not break execution
     ...
     * @return string
     */
    private function truncatePayload(?string $payload)
    {
        $maxLength = 500;
