...
use Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\RSA\PrivateKey;
use Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\RSA\PublicKey;
use Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Exception\InconsistentSetupException;
...
abstract class RSA extends AsymmetricKey
{
    ...
    public static function createKey($bits = 2048)
    {
        ...
        if ($num_primes == 2 && $bits >= 384 && self::$defaultExponent == 65537) {
            ...
            if (self::$engines['OpenSSL']) {
                ...
                return \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\RSA::load($privatekeystr);
            }
        }
        static $e;
        if (!isset($e)) {
    ...
    protected function __construct()
    {
        ...
        $this->mgfHash = new \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\Hash('sha256');
    ...
    public function withHash($hash)
    {
        ...
        switch (\strtolower($hash)) {
            ...
            case 'sha512/224':
            case 'sha512/256':
                $new->hash = new \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\Hash($hash);
                break;
            default:
    ...
    public function withMGFHash($hash)
    {
        ...
        switch (\strtolower($hash)) {
            ...
            case 'sha512/224':
            case 'sha512/256':
                $new->mgfHash = new \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\Hash($hash);
                break;
            default:
                throw new UnsupportedAlgorithmException('The only supported hash algorithms are: md2, md5, sha1, sha256, sha384, sha512, sha224, sha512/224, sha512/256');
        }
