...
class HandlerStack
{
    ...
    public function remove($remove) : void
    {
        if (!is_string($remove) && !is_callable($remove)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.4', 'Not passing a callable or string to %s::%s() is deprecated and will cause an error in 8.0.', __CLASS__, __FUNCTION__);
        }
        $this->cached = null;
        if (\is_string($remove)) {
