...
final class UriNormalizer
{
    ...
    public static function normalize(UriInterface $uri, int $flags = self::PRESERVING_NORMALIZATIONS) : UriInterface
    {
        ...
        if ($flags & self::REMOVE_DEFAULT_PORT && $uri->getPort() !== null && \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\Uri::isDefaultPort($uri)) {
            $uri = $uri->withPort(null);
        }
        if ($flags & self::REMOVE_DOT_SEGMENTS && !\Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\Uri::isRelativePathReference($uri)) {
            $uri = $uri->withPath(\Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\UriResolver::removeDotSegments($uri->getPath()));
        }
