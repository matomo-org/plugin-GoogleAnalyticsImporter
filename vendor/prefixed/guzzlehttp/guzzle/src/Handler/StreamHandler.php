...
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\ConnectException;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\RequestException;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\TransferException;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Multiplexing;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Promise as P;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Promise\FulfilledPromise;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Promise\PromiseInterface;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\TransferStats;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\TransportSharing;
...
class StreamHandler
{
    ...
    public function __construct(array $options = [])
    {
        foreach ($options as $name => $_) {
            if (!isset(self::KNOWN_CONSTRUCTOR_OPTIONS[$name])) {
                \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.14', \sprintf('The "%s" StreamHandler constructor option is unknown; guzzlehttp/guzzle 8.0 will reject unknown constructor options.', (string) $name));
            }
        }
    ...
    private static function triggerUnsupportedRequestOptionDeprecations(RequestInterface $request, array $options) : void
    {
        if (\array_key_exists('curl', $options) && $options['curl'] !== null && $options['curl'] !== [] && !self::isCurlOptionGeneratedByAuth($options)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "curl" request option to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because the stream handler ignores cURL options.');
        }
        if (\array_key_exists('expect', $options) && $options['expect'] !== \false && $request->hasHeader('Expect')) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "expect" request option to the stream handler is deprecated when it adds an Expect header; guzzlehttp/guzzle 8.0 will reject this option because the stream handler does not support Expect: 100-Continue.');
        }
    }
    private static function triggerConflictingStreamContextOptionDeprecations(array $streamContext) : void
    {
        ...
        foreach ($streamContext as $wrapper => $contextOptions) {
            ...
            foreach ($contextOptions as $option => $_) {
                ...
                \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing stream_context.%s.%s in the "stream_context" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $wrapper, $option, $conflictingOptions[$wrapper][$option]));
            }
        }
    }
    private static function triggerUnsupportedStreamContextOptionDeprecations(array $streamContext) : void
    {
        ...
        \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing PHP stream context options outside the built-in stream handler allow-list to the "stream_context" request option is deprecated; guzzlehttp/guzzle 8.0 will reject stream context options outside the allow-list. Deprecated option%s: %s.', \count($unsupportedOptions) === 1 ? '' : 's', \implode(', ', $unsupportedOptions)));
    }
    /**
     * @return string[]
     */
