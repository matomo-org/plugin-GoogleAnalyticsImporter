...
abstract class EC extends AsymmetricKey
{
    ...
    public static function createKey($curve)
    {
        ...
        if (self::$engines['libsodium'] && $curve == 'ed25519' && function_exists('sodium_crypto_sign_keypair')) {
            ...
            $privatekey = \phpseclib3\Crypt\EC::loadFormat('libsodium', sodium_crypto_sign_secretkey($kp));
        ...
        if ($curve instanceof TwistedEdwardsCurve) {
            $arr = $curve->extractSecret(\phpseclib3\Crypt\Random::string($curve instanceof Ed448 ? 57 : 32));
    ...
    public function getParameters($type = 'PKCS1')
    {
        ...
        return \phpseclib3\Crypt\EC::load($key, 'PKCS1')->withHash($this->hash->getHash())->withSignatureFormat($this->shortFormat);
    }
    /**
