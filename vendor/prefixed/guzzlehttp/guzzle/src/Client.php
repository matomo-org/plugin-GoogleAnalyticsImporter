...
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Cookie\CookieJar;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Cookie\CookieJarInterface;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\GuzzleException;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\InvalidArgumentException;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Handler\CurlShareHandleState;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Handler\CurlVersion;
 ...
 * @final
 */
class Client implements ClientInterface, \Matomo\Dependencies\GoogleAnalyticsImporter\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
    ...
    {
        \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.1', '%s::%s() is deprecated and will be removed in 8.0.', __CLASS__, __FUNCTION__);
        if (\count($args) < 1) {
    ...
    public function requestAsync(string $method, $uri = '', array $options = []) : PromiseInterface
    {
        ...
        if ($method !== $normalizedMethod) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::requestAsync() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
    ...
    public function request(string $method, $uri = '', array $options = []) : ResponseInterface
    {
        $normalizedMethod = Psr7\Utils::asciiToUpper($method);
        if ($method !== $normalizedMethod) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::request() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
            $method = $normalizedMethod;
        }
        $options[RequestOptions::SYNCHRONOUS] = \true;
    ...
    private static function warnAboutRequestLevelHandler(array $options) : void
    {
        ...
        \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing the "handler" request option is deprecated; guzzlehttp/guzzle 8.0 will ignore request-level handlers. Configure the handler when creating the Client, or use a separate Client instance for requests that need a different handler.');
    }
    ...
    {
        \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', $since, 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
    ...
    private function transfer(RequestInterface $request, array $options) : PromiseInterface
    {
        ...
        if ('' === $protocolVersion) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with an empty protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            $request = Psr7\Utils::modifyRequest($request, ['version' => '1.1']);
        } elseif (!self::isProtocolVersionValid($protocolVersion)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with a malformed protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject malformed protocol versions.');
        }
        ...
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                ...
                case 'digest':
                    ...
                    break;
                case 'ntlm':
                    \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing "ntlm" as the built-in auth type is deprecated; guzzlehttp/guzzle 8.0 will no longer apply NTLM through the "auth" request option. NTLM is also deprecated by curl/libcurl and may be unavailable in current or future libcurl builds. Avoid NTLM; if you must use it temporarily, configure cURL HTTP authentication options directly with a libcurl build that still supports NTLM.');
    ...
    private static function createBodyStream($body) : StreamInterface
    {
        ...
        if (\is_scalar($body)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-string scalar to the "body" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-string scalar bodies.');
    ...
    private static function normalizeNonFiniteFloats(array $values, string $option) : array
    {
        foreach ($values as $key => $value) {
            if (\is_array($value)) {
                ...
                \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-finite float in the "%s" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-finite floats.', $option);
    ...
    {
        if ('' === $version) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing an empty "version" request option is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            return '1.1';
        }
