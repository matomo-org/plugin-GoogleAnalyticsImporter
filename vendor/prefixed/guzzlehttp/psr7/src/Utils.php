...
final class Utils
{
    ...
    {
        return strpos(self::asciiToLower($haystack), self::asciiToLower($needle)) !== false;
    }
    /**
    ...
    {
        \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s to Utils::modifyRequest() change "%s" is deprecated; guzzlehttp/psr7 3.0 requires %s.', \get_debug_type($value), $key, $expected);
    }
    /**
    ...
    public static function streamFor($resource = '', array $options = []) : StreamInterface
    {
        if (is_scalar($resource)) {
            if (!is_string($resource)) {
                \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.12', 'Passing %s to Utils::streamFor() is deprecated; cast it to a string. guzzlehttp/psr7 3.0 will only accept string, resource, StreamInterface, Stringable, Iterator, callable, or null.', \gettype($resource));
