...
//        Metrics::INDEX_PAGE_EXIT_NB_VISITS, Not available in GA4
    ];
...
//            Archiver::METRIC_SUM_TIME_RECORD_NAME       => array_sum($pageUrls->getColumn(Metrics::INDEX_PAGE_SUM_TIME_GENERATION)), Not available in GA4
//            Archiver::METRIC_HITS_TIMED_RECORD_NAME     => array_sum($pageUrls->getColumn(Metrics::INDEX_PAGE_NB_HITS_WITH_TIME_GENERATION)) Not available in GA4
        );
    ...
    private function getSiteSearchs(Date $day)
    {
        ...
        $table = $gaQuery->query($day, $dimensions = ['searchTerm'], $metrics, [
            'orderBys' => [
//                ['field' => 'ga:searchUniques', 'order' => 'descending'], Not available in GA4
                ['field' => 'searchTerm', 'order' => 'ascending'],
            ],
            'mappings' => [
//                Metrics::INDEX_NB_VISITS => 'ga:searchUniques', Not available in GA4
//                Metrics::INDEX_PAGE_NB_HITS => 'ga:searchResultViews', Not available in GA4
            ],
        ]);
    ...
    private function getPageTitlesRecord(Date $day)
    {
        ...
        if ($this->isMobileApp) {
            $table = $gaQuery->query($day, $dimensions = [$this->pageTitleDimension], $this->getPageMetrics(), [
                'orderBys' => [
//                    ['field' => 'ga:screenviews', 'order' => 'descending'], Not available in GA4
//                    ['field' => 'ga:screenName', 'order' => 'ascending'], Not available in GA4
                ],
                'mappings' => [
            ...
            $table = $gaQuery->query($day, $dimensions = [$this->pageTitleDimension, 'pagePath'], $this->getPageMetrics(), [
                'orderBys' => [
...
//                ['field' => $this->uniquePageviewsMetric, 'order' => 'descending'], Not available in GA4
            ...
            'mappings' => [
//                Metrics::INDEX_NB_VISITS => $this->uniquePageviewsMetric, Not available in GA4
            ],
        ]);
    ...
    private function getPageUrlsRecord(Date $day)
    {
        ...
        $table = $gaQuery->query($day, $dimensions = ['pagePath'], $this->getPageMetrics(), [
            'orderBys' => [
                ['field' => 'screenPageViews', 'order' => 'descending'],
        ...
        $metrics = [Metrics::INDEX_NB_VISITS, Metrics::INDEX_NB_UNIQ_VISITORS];
        $table = $gaQuery->query($day, $dimensions = ['pagePath'], $metrics, [
            'orderBys' => [
//                ['field' => 'ga:uniquePageviews', 'order' => 'descending'],  Not available in GA4
                ['field' => 'pagePath', 'order' => 'ascending'],
            ],
            'mappings' => [
//                Metrics::INDEX_NB_VISITS => 'ga:uniquePageviews',  Not available in GA4
            ],
        ]);
    ...
    private function queryEntryPagesForUrls(Date $day)
    {
        ...
        $gaQuery = $this->getGaClient();
        $table = $gaQuery->query($day, $dimensions = ['landingPage'], $this->entryPageMetrics, [
            'orderBys' => [
