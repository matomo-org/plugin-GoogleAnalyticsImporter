...
class JWT
{
        ...
        $key, string $alg) : string
    {
        ...
        switch ($function) {
            ...
            case 'openssl':
                ...
                if (strncmp($alg, 'RS', strlen('RS')) === 0) {
                    self::validateRsaKeyLength($key);
                } elseif (strncmp($alg, 'ES', strlen('ES')) === 0) {
        ...
        $key, string $message) : string
    {
        ...
        if (is_resource($key) || $key instanceof OpenSSLCertificate) {
        ...
        $rsa = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\PublicKeyLoader::load($key);
        return $rsa->withPadding(\Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\RSA::SIGNATURE_PSS)->withHash('sha256')->sign($message);
    }
    /**
        ...
        $key, string $message, string $signature) : bool
    {
        ...
        if ($key instanceof OpenSSLAsymmetricKey) {
            ...
            $key = $details['key'];
        } elseif (is_resource($key) || $key instanceof OpenSSLCertificate) {
        ...
        $rsa = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\PublicKeyLoader::load($key);
        return $rsa->withPadding(\Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\RSA::SIGNATURE_PSS)->withHash('sha256')->verify($message, $signature);
    }
}
