...
use Matomo\Dependencies\GoogleAnalyticsImporter\Google\Cloud\Firestore\V1beta1\FirestoreGrpcClient;
use Matomo\Dependencies\GoogleAnalyticsImporter\Google\Cloud\Spanner\V1\SpannerGrpcClient;
function getArgs()
{
...
function executeProbes($api)
{
    ...
    $util = new \Matomo\Dependencies\GoogleAnalyticsImporter\StackdriverUtil($api);
    $auth = \Matomo\Dependencies\GoogleAnalyticsImporter\Google\Auth\ApplicationDefaultCredentials::getCredentials($_OAUTH_SCOPE);
    ...
    if ($success == $total) {
        $util->setSuccess(\Matomo\Dependencies\GoogleAnalyticsImporter\True);
    }
