 ...
 */
namespace Matomo\Dependencies\GoogleAnalyticsImporter\Google\AccessToken;

use DomainException;
use Exception;
use Matomo\Dependencies\GoogleAnalyticsImporter\Firebase\JWT\CachedKeySet;
use Matomo\Dependencies\GoogleAnalyticsImporter\Firebase\JWT\ExpiredException;
...
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\ClientInterface as GuzzleClientInterface;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\HttpFactory;
...
class Verify
{
     ...
     */
    public function __construct(?GuzzleClientInterface $http = null, ?CacheItemPoolInterface $cache = null, ?JWT $jwt = null)
    {
        if (null === $http) {
            $http = new Client();
        }
        ...
        $this->keySet = new CachedKeySet(self::FEDERATED_SIGNON_CERT_URL, $http, new HttpFactory(), $cache);
    }
    /**
    ...
    public function verifyIdToken($idToken, $audience = null)
    {
        ...
        } catch (ExpiredException|SignatureInvalidException|DomainException $exception) {
            return \false;
        }
