...
class TasksTest extends IntegrationTestCase
{
    ...
    public function test_resumeScheduledImports_runANormalStatusCommandCorrectly()
    {
        ...
        $tasks->resumeScheduledImports();
        $this->assertEquals([
            [false, 'nohup ' . $this->getPhpBinary() . ' ' . PIWIK_INCLUDE_PATH . "/tests/PHPUnit/proxy/console{$this->getCommandHostOption()} googleanalyticsimporter:import-reports --idsite=1 >> " . PIWIK_INCLUDE_PATH . '/tmp/logs/gaimportlog.1.mamp.log 2>&1 &'],
    ...
    public function test_resumeScheduledImports_runAStatusWithVerboseLoggingCorrectly()
    {
        ...
        $tasks->resumeScheduledImports();
        $this->assertEquals([
            [false, 'nohup ' . $this->getPhpBinary() . ' ' . PIWIK_INCLUDE_PATH . "/tests/PHPUnit/proxy/console{$this->getCommandHostOption()} googleanalyticsimporter:import-reports --idsite=1 -vvv > " . PIWIK_INCLUDE_PATH . '/tmp/logs/gaimportlog.1.mamp.log 2>&1 &'],
        ], TasksWithMockExec::$commandsRun);
    }
