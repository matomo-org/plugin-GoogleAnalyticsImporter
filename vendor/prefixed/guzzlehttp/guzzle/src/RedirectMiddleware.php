...
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\BadResponseException;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\TooManyRedirectsException;
...
class RedirectMiddleware
{
    ...
    public function checkRedirect(RequestInterface $request, array $options, ResponseInterface $response)
    {
        ...
        if (\Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\UriComparator::isCrossOrigin($request->getUri(), $nextRequest->getUri()) && \defined('\\CURLOPT_HTTPAUTH')) {
    ...
    public function modifyRequest(RequestInterface $request, array $options, ResponseInterface $response) : RequestInterface
    {
        ...
        $modify['uri'] = $uri;
        \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\Message::rewindBody($request);
        ...
        if (\Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\UriComparator::isCrossOrigin($request->getUri(), $modify['uri'])) {
            $modify['remove_headers'][] = 'Authorization';
            $modify['remove_headers'][] = 'Cookie';
        }
        return \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify);
    }
    /**
    ...
    {
        $location = \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\UriResolver::resolve($request->getUri(), new \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\Uri($response->getHeaderLine('Location')));
