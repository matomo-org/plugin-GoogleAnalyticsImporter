...
use Matomo\Dependencies\GoogleAnalyticsImporter\ParagonIE\ConstantTime\Base64;
use Matomo\Dependencies\GoogleAnalyticsImporter\ParagonIE\ConstantTime\Hex;
use Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\Common\PrivateKey;
...
use Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\EC;
use Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\Hash;
...
use Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Crypt\RSA\Formats\Keys\PSS;
use Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Exception\UnsupportedAlgorithmException;
...
class X509
{
    ...
    public function __construct()
    {
        ...
        if (!self::$oidsLoaded) {
            ...
            \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::loadOIDs([
    ...
    public function loadX509($cert, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcert = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::extractBER($cert);
        ...
        $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($cert);
        if (!empty($decoded)) {
            $x509 = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], Maps\Certificate::MAP);
        }
        ...
        $key = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($key, Maps\SubjectPublicKeyInfo::MAP);
    ...
    public function saveX509($cert, $format = self::FORMAT_PEM)
    {
        ...
        $filters = [];
        $type_utf8_string = ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        ...
        /* in the case of policyQualifiers/qualifier, the type has to be \phpseclib3\File\ASN1::TYPE_IA5_STRING.
             ...
             characters.
           */
        $filters['policyQualifiers']['qualifier'] = ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_IA5_STRING];
        \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::setFilters($filters);
        ...
        $cert = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($cert, Maps\Certificate::MAP);
        switch ($format) {
        ...
        if ($extensions) {
            for ($i = 0; $i < \count($extensions); $i++) {
                ...
                if (!\is_bool($map)) {
                    ...
                    $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($value);
                    $mapped = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], $map, ['iPAddress' => $decoder]);
                    ...
                    if ($id == 'id-ce-certificatePolicies') {
                        for ($j = 0; $j < \count($value); $j++) {
                            ...
                            for ($k = 0; $k < \count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                if ($map !== \false) {
                                    $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($subvalue);
                                    $mapped = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
        ...
        if (\is_array($extensions)) {
            ...
            for ($i = 0; $i < $size; $i++) {
                        ...
                        for ($j = 0; $j < \count($value); $j++) {
                            ...
                            for ($k = 0; $k < \count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                if ($map !== \false) {
                                    ...
                                    $subvalue = new Element(\Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($subvalue, $map));
                                }
                            }
                        }
                        break;
                    case 'id-ce-authorityKeyIdentifier':
                        ...
                        if (isset($value['authorityCertSerialNumber'])) {
                            if ($value['authorityCertSerialNumber']->toBytes() == '') {
                                $temp = \chr(\Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::CLASS_CONTEXT_SPECIFIC << 6 | 2) . "\x01\x00";
                ...
                if (\is_bool($map)) {
                    ...
                    $value = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($value, $map, ['iPAddress' => [static::class, 'encodeIP']]);
                }
            }
        }
    }
            ...
            for ($i = 0; $i < \count($attributes); $i++) {
                ...
                if (\is_array($attributes[$i]['value'])) {
                    ...
                    for ($j = 0; $j < \count($values); $j++) {
                        $value = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($values[$j], Maps\AttributeValue::MAP);
                        $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($value);
                        if (!\is_bool($map)) {
                            $mapped = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
                            if ($mapped !== \false) {
            ...
            for ($i = 0; $i < $size; $i++) {
                ...
                if ($map === \false) {
                    ...
                    for ($j = 0; $j < \count($values); $j++) {
                        ...
                        if (!\is_bool($map)) {
                            $temp = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($values[$j], $map);
                            $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($temp);
                            $values[$j] = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], Maps\AttributeValue::MAP);
                        }
                    }
                }
            }
            ...
            for ($i = 0; $i < \count($dns); $i++) {
                for ($j = 0; $j < \count($dns[$i]); $j++) {
                    ...
                    if (\is_object($value) && $value instanceof Element) {
                        ...
                        if (!\is_bool($map)) {
                            $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($value);
                            $value = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
                        }
                    }
                }
            }
        ...
        if (\is_array($dns)) {
            ...
            for ($i = 0; $i < $size; $i++) {
                for ($j = 0; $j < \count($dns[$i]); $j++) {
                    ...
                    $map = $this->getMapping($type);
                    if (!\is_bool($map)) {
                        $value = new Element(\Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($value, $map));
                    }
                }
            }
        }
    ...
    private function validateSignatureCountable($caonly, $count)
    {
        ...
        switch (\true) {
            case isset($this->currentCert['tbsCertificate']):
                // self-signed cert
                switch (\true) {
                    case !\defined('Matomo\\Dependencies\\GoogleAnalyticsImporter\\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertificate']['issuer'] === $this->currentCert['tbsCertificate']['subject']:
                    case \defined('Matomo\\Dependencies\\GoogleAnalyticsImporter\\FILE_X509_IGNORE_TYPE') && $this->getIssuerDN(self::DN_STRING) === $this->getDN(self::DN_STRING):
                ...
                if (!empty($this->CAs)) {
                    for ($i = 0; $i < \count($this->CAs); $i++) {
                        ...
                        $ca = $this->CAs[$i];
                        switch (\true) {
                            case !\defined('Matomo\\Dependencies\\GoogleAnalyticsImporter\\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertificate']['issuer'] === $ca['tbsCertificate']['subject']:
                            case \defined('Matomo\\Dependencies\\GoogleAnalyticsImporter\\FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING, $this->currentCert['tbsCertificate']['issuer']) === $this->getDN(self::DN_STRING, $ca['tbsCertificate']['subject']):
                                $authorityKey = $this->getExtension('id-ce-authorityKeyIdentifier');
            ...
            case isset($this->currentCert['tbsCertList']):
                if (!empty($this->CAs)) {
                    for ($i = 0; $i < \count($this->CAs); $i++) {
                        $ca = $this->CAs[$i];
                        switch (\true) {
                            case !\defined('Matomo\\Dependencies\\GoogleAnalyticsImporter\\FILE_X509_IGNORE_TYPE') && $this->currentCert['tbsCertList']['issuer'] === $ca['tbsCertificate']['subject']:
                            case \defined('Matomo\\Dependencies\\GoogleAnalyticsImporter\\FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING, $this->currentCert['tbsCertList']['issuer']) === $this->getDN(self::DN_STRING, $ca['tbsCertificate']['subject']):
                                $authorityKey = $this->getExtension('id-ce-authorityKeyIdentifier');
                                $subjectKeyID = $this->getExtension('id-ce-subjectKeyIdentifier', $ca);
                                switch (\true) {
                                    case !\is_array($authorityKey):
    ...
    public function getDNProp($propName, $dn = null, $withType = \false)
    {
        ...
        $filters = [];
        $filters['value'] = ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::setFilters($filters);
        ...
        for ($i = 0; $i < \count($dn); $i++) {
            if ($dn[$i][0]['type'] == $propName) {
                ...
                if (!$withType) {
                    if (\is_array($v)) {
                        foreach ($v as $type => $s) {
                            $type = \array_search($type, \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::ANY_MAP);
                            if ($type !== \false && \array_key_exists($type, \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::STRING_TYPE_SIZE)) {
                                $s = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::convert($s, $type);
                                if ($s !== \false) {
                                    $v = $s;
                                    break;
                                }
                        ...
                        if (!\is_bool($map)) {
                            $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($v);
                            $v = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], $map);
                        }
                    }
                }
                $result[] = $v;
    ...
    public function getDN($format = self::DN_ARRAY, $dn = null)
    {
        ...
        switch ((int) $format) {
            ...
            case self::DN_ASN1:
                $filters = [];
                $filters['rdnSequence']['value'] = ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
                \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::setFilters($filters);
                $this->mapOutDNs($dn, 'rdnSequence');
                return \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($dn, Maps\Name::MAP);
            case self::DN_CANON:
                ...
                foreach ($dn['rdnSequence'] as $rdn) {
                    foreach ($rdn as $i => $attr) {
                        ...
                        if (\is_array($attr['value'])) {
                            foreach ($attr['value'] as $type => $v) {
                                $type = \array_search($type, \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::ANY_MAP, \true);
                                if ($type !== \false && \array_key_exists($type, \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::STRING_TYPE_SIZE)) {
                                    $v = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::convert($v, $type);
                                    if ($v !== \false) {
                                        $v = \preg_replace('/\\s+/', ' ', $v);
                    ...
                    $result .= \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($rdn, Maps\RelativeDistinguishedName::MAP);
                }
                return $result;
            case self::DN_HASH:
    ...
    public function getChain()
    {
        ...
        foreach ($chain as $key => $value) {
            $chain[$key] = new \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\X509();
    ...
    public function loadCSR($csr, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcsr = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::extractBER($csr);
        ...
        $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($csr);
        if (empty($decoded)) {
            $this->currentCert = \false;
            return \false;
        }
        $csr = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], Maps\CertificationRequest::MAP);
    ...
    public function saveCSR($csr, $format = self::FORMAT_PEM)
    {
        ...
        $filters = [];
        $filters['certificationRequestInfo']['subject']['rdnSequence']['value'] = ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::setFilters($filters);
        ...
        $csr = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($csr, Maps\CertificationRequest::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $csr;
            // case self::FORMAT_PEM:
    ...
    public function loadSPKAC($spkac)
    {
        ...
        $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($spkac);
        if (empty($decoded)) {
            $this->currentCert = \false;
            return \false;
        }
        $spkac = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], Maps\SignedPublicKeyAndChallenge::MAP);
    ...
    public function saveSPKAC($spkac, $format = self::FORMAT_PEM)
    {
        ...
        $spkac = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($spkac, Maps\SignedPublicKeyAndChallenge::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $spkac;
            // case self::FORMAT_PEM:
    ...
    public function loadCRL($crl, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcrl = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::extractBER($crl);
        ...
        $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($crl);
        if (empty($decoded)) {
            $this->currentCert = \false;
            return \false;
        }
        $crl = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], Maps\CertificateList::MAP);
    ...
    public function saveCRL($crl, $format = self::FORMAT_PEM)
    {
        ...
        $filters = [];
        $filters['tbsCertList']['issuer']['rdnSequence']['value'] = ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        $filters['tbsCertList']['signature']['parameters'] = ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        $filters['signatureAlgorithm']['parameters'] = ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        if (empty($crl['tbsCertList']['signature']['parameters'])) {
            $filters['tbsCertList']['signature']['parameters'] = ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_NULL];
        }
        if (empty($crl['signatureAlgorithm']['parameters'])) {
            $filters['signatureAlgorithm']['parameters'] = ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_NULL];
        }
        \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::setFilters($filters);
        ...
        $crl = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeDER($crl, Maps\CertificateList::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $crl;
            // case self::FORMAT_PEM:
    ...
    public function setEndDate($date)
    {
        ...
        */
        if (\is_string($date) && \strtolower($date) === 'lifetime') {
            ...
            $temp = \chr(\Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_GENERALIZED_TIME) . \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::encodeLength(\strlen($temp)) . $temp;
    ...
    public function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        switch (\true) {
            ...
            case $key instanceof Element:
                ...
                $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($key->element);
                if (empty($decoded)) {
                    return \false;
                }
                $raw = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], ['type' => \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::TYPE_BIT_STRING]);
                if (empty($raw)) {
                    return \false;
                }
                ...
                // Is a public key.
                break;
            case $key instanceof \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\X509:
        ...
        $key = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::extractBER($key);
    ...
    private function formatSubjectPublicKey()
    {
        ...
        $decoded = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::decodeBER($publicKey);
        $mapped = \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\File\ASN1::asn1map($decoded[0], Maps\SubjectPublicKeyInfo::MAP);
        if (!\is_array($mapped)) {
            return \false;
        }
