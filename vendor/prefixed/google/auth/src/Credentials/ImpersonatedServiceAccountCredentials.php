...
class ImpersonatedServiceAccountCredentials extends CredentialsLoader implements SignBlobInterface, GetUniverseDomainInterface, UpdateMetadataInterface
{
    ...
    /**
     * @var \Matomo\Dependencies\GoogleAnalyticsImporter\Google\Auth\FetchAuthTokenInterface
     */
     ...
     *                                           in the `$jsonKey`.
     */
    public function __construct($scope, $jsonKey, ?string $targetAudience = null, $defaultScope = null, bool $enableRegionalAccessBoundary = \false)
    {
        ...
        if (is_array($jsonKey['source_credentials'])) {
            ...
            switch ($jsonKey['source_credentials']['type'] ?? null) {
                case 'service_account':
                    $jsonKey['source_credentials'] = new ServiceAccountCredentials($scope, $jsonKey['source_credentials']);
                    break;
                case 'authorized_user':
                    $jsonKey['source_credentials'] = new UserRefreshCredentials($scope, $jsonKey['source_credentials']);
                    break;
                case 'external_account':
                    $jsonKey['source_credentials'] = new ExternalAccountCredentials($scope, $jsonKey['source_credentials']);
                    break;
                default:
    ...
    public function fetchAuthToken(?callable $httpHandler = null)
    {
        ...
        switch ($this->isIdTokenRequest()) {
            case \true:
                $body = ['audience' => $this->targetAudience, 'includeEmail' => \true];
                break;
            case \false:
                $body = ['scope' => $this->targetScope, 'delegates' => $this->delegates, 'lifetime' => sprintf('%ss', $this->lifetime)];
                break;
        }
    ...
    public function updateMetadata($metadata, $authUri = null, ?callable $httpHandler = null)
    {
        ...
        $metatadata = $this->updateRegionalAccessBoundaryMetadata($metatadata, $this->buildRegionalAccessBoundaryLookupUrl($this->impersonatedServiceAccountName), $this->getUniverseDomain(), $httpHandler);
        return $metatadata;
    }
}
