...
class Request implements RequestInterface
{
    ...
    public function withUri(UriInterface $uri, $preserveHost = \false) : RequestInterface
    {
        if (!\is_bool($preserveHost)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s to RequestInterface::withUri() is deprecated; guzzlehttp/psr7 3.0 requires bool for $preserveHost.', \get_debug_type($preserveHost));
        }
        if ($uri === $this->uri) {
            return $this;
        }
    ...
    private static function warnOnMethodCasingChange(string $method) : void
    {
        if ($method !== Utils::asciiToUpper($method)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing a non-uppercase HTTP method is deprecated; guzzlehttp/psr7 3.0 preserves method casing and will no longer uppercase it. Normalize the method before constructing or modifying requests if uppercase is required.');
        }
    }
}
