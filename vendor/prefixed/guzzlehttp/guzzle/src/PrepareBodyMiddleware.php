    ...
    public function __invoke(RequestInterface $request, array $options) : PromiseInterface
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if (\is_string($uri) && ($type = \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\MimeType::fromFilename($uri))) {
        ...
        return $fn(\Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify), $options);
    }
    /**
     * Add expect header
     */
