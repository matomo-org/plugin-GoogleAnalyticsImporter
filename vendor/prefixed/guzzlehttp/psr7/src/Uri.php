...
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\Exception\MalformedUriException;
...
class Uri implements UriInterface, \JsonSerializable
{
    ...
    {
        if (!is_string($value)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.12', 'Passing %s to Uri::withQueryValues() is deprecated; cast it to a string. guzzlehttp/psr7 3.0 will only accept string or null query values.', \gettype($value));
    ...
    public function withPort($port) : UriInterface
    {
        if ($port !== null && !\is_int($port)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s to UriInterface::withPort() is deprecated; guzzlehttp/psr7 3.0 requires int|null.', \get_debug_type($port));
        }
    ...
    private function filterScheme($scheme) : string
    {
        ...
        if ($scheme !== '' && !preg_match('/^[a-z][a-z0-9.+-]*$/D', $scheme)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing "%s" as a URI scheme is deprecated; guzzlehttp/psr7 3.0 requires URI schemes to match RFC 3986 syntax and begin with a letter.', $scheme);
        }
        return $scheme;
    }
    /**
