...
final class MultipartStream implements StreamInterface
{
    ...
    public function __construct(array $elements = [], ?string $boundary = null)
    {
        if ($boundary !== null && !self::isValidBoundary($boundary)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing an invalid multipart boundary to MultipartStream::__construct() is deprecated; guzzlehttp/psr7 3.0 rejects invalid multipart boundaries.');
        }
    ...
    private function addElement(AppendStream $stream, array $element) : void
    {
        ...
        if (is_scalar($contents) && !is_string($contents)) {
            ...
            if (is_float($contents) && !is_finite($contents)) {
                \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.12', 'Passing a non-finite float as multipart contents is deprecated; guzzlehttp/psr7 3.0 rejects non-finite floats.');
    ...
    private static function normalizePartHeaders(array $headers) : array
    {
        ...
        foreach ($headers as $key => $value) {
            ...
            if (!is_string($value)) {
                ...
                \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s as a multipart part header value is deprecated; guzzlehttp/psr7 3.0 requires string multipart part header values.', \get_debug_type($value));
            }
            $value = (string) $value;
    ...
    private static function deprecateInvalidPartHeaderName(string $name) : void
    {
        if (!preg_match('/^[a-zA-Z0-9\'`#$%&*+.^_|~!-]+$/D', $name)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing an invalid multipart part header name to MultipartStream is deprecated; guzzlehttp/psr7 3.0 rejects invalid multipart part header names.');
        }
    }
    private static function deprecateInvalidPartHeaderValue(string $value) : void
    {
        if (!preg_match('/^[\\x20\\x09\\x21-\\x7E\\x80-\\xFF]*$/D', $value)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing an invalid multipart part header value to MultipartStream is deprecated; guzzlehttp/psr7 3.0 rejects invalid multipart part header values.');
        }
    }
}
