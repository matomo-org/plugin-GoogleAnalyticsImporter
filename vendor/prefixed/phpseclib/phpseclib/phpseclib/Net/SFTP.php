...
use Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Exception\FileNotFoundException;
/**
 ...
 * @access  public
 */
class SFTP extends \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Net\SSH2
{
    /**
    ...
    public function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        if (!\defined('Matomo\\Dependencies\\GoogleAnalyticsImporter\\NET_SFTP_QUEUE_SIZE')) {
            \define('Matomo\\Dependencies\\GoogleAnalyticsImporter\\NET_SFTP_QUEUE_SIZE', 32);
        }
        if (!\defined('Matomo\\Dependencies\\GoogleAnalyticsImporter\\NET_SFTP_UPLOAD_QUEUE_SIZE')) {
            \define('Matomo\\Dependencies\\GoogleAnalyticsImporter\\NET_SFTP_UPLOAD_QUEUE_SIZE', 1024);
        }
    }
    /**
    ...
    {
        if (!($this->bitmap & \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Net\SSH2::MASK_LOGIN)) {
            return \false;
        }
        if ($this->pwd === \false) {
    ...
    public function put($remote_file, $data, $mode = self::SOURCE_STRING, $start = -1, $local_start = -1, $progressCallback = null)
    {
        ...
        if ($mode & \Matomo\Dependencies\GoogleAnalyticsImporter\phpseclib3\Net\SFTP::SOURCE_LOCAL_FILE) {
    ...
    private function send_sftp_packet($type, $data, $request_id = 1)
    {
        ...
        $stop = \microtime(\true);
        if (\defined('Matomo\\Dependencies\\GoogleAnalyticsImporter\\NET_SFTP_LOGGING')) {
