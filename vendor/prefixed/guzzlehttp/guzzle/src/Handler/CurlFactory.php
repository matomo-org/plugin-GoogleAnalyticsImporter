...
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Psr7\Uri;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function isCurlShareHandle($value) : bool
    {
        ...
        return is_resource($value) || $value instanceof \CurlShareHandle;
    }
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options) : void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function triggerUnsupportedCurlOptionDeprecations(array $options) : void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject raw cURL options outside the built-in cURL handlers\' allow-list.', self::formatCurlOption($option)));
        }
    }
    private static function triggerUnsupportedRequestOptionDeprecations(array $options) : void
    {
        if (\array_key_exists('stream_context', $options)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "stream_context" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because cURL handlers ignore PHP stream context options.');
        }
    }
    /**
    ...
    public function release(EasyHandle $easy) : void
    {
        ...
        if (\defined('Matomo\\Dependencies\\GoogleAnalyticsImporter\\CURLOPT_PREREQFUNCTION')) {
