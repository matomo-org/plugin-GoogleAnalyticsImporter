...
class JWT
{
    ...
    public static function decode(string $jwt, $keyOrKeyArray) : stdClass
    {
        ...
        if (!self::verify("{$headb64}.{$bodyb64}", $sig, $key->getKeyMaterial(), $header->alg)) {
            throw new \Matomo\Dependencies\GoogleAnalyticsImporter\Firebase\JWT\SignatureInvalidException('Signature verification failed');
        }
        // Check the nbf if it is defined. This is the time that the
        // token can actually be used. If it's not yet that time, abort.
        if (isset($payload->nbf) && $payload->nbf > $timestamp + static::$leeway) {
            throw new \Matomo\Dependencies\GoogleAnalyticsImporter\Firebase\JWT\BeforeValidException('Cannot handle token prior to ' . \date(DateTime::ISO8601, $payload->nbf));
        }
        // Check that this token has been created before 'now'. This prevents
        // using tokens that have been created for later use (and haven't
        // correctly used the nbf claim).
        if (isset($payload->iat) && $payload->iat > $timestamp + static::$leeway) {
            throw new \Matomo\Dependencies\GoogleAnalyticsImporter\Firebase\JWT\BeforeValidException('Cannot handle token prior to ' . \date(DateTime::ISO8601, $payload->iat));
        }
        // Check if this token has expired.
        if (isset($payload->exp) && $timestamp - static::$leeway >= $payload->exp) {
            throw new \Matomo\Dependencies\GoogleAnalyticsImporter\Firebase\JWT\ExpiredException('Expired token');
        }
        return $payload;
    }
    /**
     ...
     * @return Key
     */
    private static function getKey($keyOrKeyArray, ?string $kid) : \Matomo\Dependencies\GoogleAnalyticsImporter\Firebase\JWT\Key
    {
        if ($keyOrKeyArray instanceof \Matomo\Dependencies\GoogleAnalyticsImporter\Firebase\JWT\Key) {
            return $keyOrKeyArray;
        }
        if ($keyOrKeyArray instanceof \Matomo\Dependencies\GoogleAnalyticsImporter\Firebase\JWT\CachedKeySet) {
