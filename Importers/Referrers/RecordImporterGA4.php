...
class RecordImporterGA4 extends \Piwik\Plugins\GoogleAnalyticsImporter\RecordImporterGA4
{
    ...
    public function importRecords(Date $day)
    {
        ...
        $this->insertRecord(Archiver::CAMPAIGNS_RECORD_NAME, $keywordByCampaign, $this->maximumRowsInDataTableLevelZero, $this->maximumRowsInSubDataTable, $this->columnToSortByBeforeTruncation);
    ...
    private function getKeywordByCampaign(Date $day)
    {
        ...
        $table = $gaQuery->query($day, $dimensions = ['sessionCampaignName', 'sessionGoogleAdsKeyword'], $this->getConversionAwareVisitMetrics());
        $keywordByCampaign = new DataTable();
        foreach ($table->getRows() as $row) {
            $campaign = $row->getMetadata('sessionCampaignName');
            if (empty($campaign)) {
                continue;
            }
            $keyword = $row->getMetadata('sessionGoogleAdsKeyword');
            if (empty($keyword)) {
                $keyword = self::NOT_SET_IN_GA_LABEL;
            }
            $this->campaignKeywords[$keyword] = \true;
    ...
    private function getUrlByWebsite(Date $day)
    {
        ...
        $gaQuery = $this->getGaClient();
        $table = $gaQuery->query($day, $dimensions = ['pageReferrer', 'sessionMedium'], $this->getConversionAwareVisitMetrics());
        $urlByWebsite = new DataTable();
        $urlBySocialNetwork = new DataTable();
        foreach ($table->getRows() as $row) {
            $referrerUrl = $row->getMetadata('pageReferrer');
            if ($referrerUrl == '(direct)') {
                continue;
            }
            $medium = $row->getMetadata('sessionMedium');
            if ($medium != 'referral') {
                continue;
            }
            ...
            if (!empty($socialNetwork) && $socialNetwork !== Piwik::translate('General_Unknown')) {
    ...
    private function getKeywordsAndSearchEngineRecords(Date $day)
    {
        ...
        $gaQuery = $this->getGaClient();
        $table = $gaQuery->query($day, $dimensions = ['sessionSource', 'sessionMedium', 'sessionGoogleAdsKeyword', 'sessionCampaignName'], $this->getConversionAwareVisitMetrics());
        foreach ($table->getRows() as $row) {
            $source = $row->getMetadata('sessionSource');
            $medium = $row->getMetadata('sessionMedium');
            $keyword = $row->getMetadata('sessionGoogleAdsKeyword');
            $campaign = $row->getMetadata('sessionCampaignName');
            if ($medium == 'referral') {
    ...
    {
        $gaQuery = $this->getGaClient();
        $table = $gaQuery->query($day, $dimensions = ['sessionSource'], $this->getConversionAwareVisitMetrics());
        foreach ($table->getRows() as $row) {
            $source = $row->getMetadata('sessionSource');
