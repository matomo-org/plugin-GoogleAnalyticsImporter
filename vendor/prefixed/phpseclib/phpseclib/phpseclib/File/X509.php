...
class X509
{
    ...
    public function loadX509($cert, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcert = \phpseclib3\File\ASN1::extractBER($cert);
        ...
        $decoded = \phpseclib3\File\ASN1::decodeBER($cert);
        if ($decoded) {
            $x509 = \phpseclib3\File\ASN1::asn1map($decoded[0], Maps\Certificate::MAP);
        }
        ...
        $key = \phpseclib3\File\ASN1::encodeDER($key, Maps\SubjectPublicKeyInfo::MAP);
    ...
    public function saveX509(array $cert, $format = self::FORMAT_PEM)
    {
        ...
        $filters = [];
        $type_utf8_string = ['type' => \phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        ...
        /* in the case of policyQualifiers/qualifier, the type has to be \phpseclib3\File\ASN1::TYPE_IA5_STRING.
             ...
             characters.
           */
        $filters['policyQualifiers']['qualifier'] = ['type' => \phpseclib3\File\ASN1::TYPE_IA5_STRING];
        \phpseclib3\File\ASN1::setFilters($filters);
        ...
        $cert = \phpseclib3\File\ASN1::encodeDER($cert, Maps\Certificate::MAP);
        switch ($format) {
        ...
        if ($extensions) {
            for ($i = 0; $i < count($extensions); $i++) {
                ...
                if (!is_bool($map)) {
                    ...
                    $decoded = \phpseclib3\File\ASN1::decodeBER($value);
                    if (!$decoded) {
                        continue;
                    }
                    $mapped = \phpseclib3\File\ASN1::asn1map($decoded[0], $map, ['iPAddress' => $decoder]);
                    ...
                    if ($id == 'id-ce-certificatePolicies') {
                        for ($j = 0; $j < count($value); $j++) {
                            ...
                            for ($k = 0; $k < count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                if ($map !== \false) {
                                    $decoded = \phpseclib3\File\ASN1::decodeBER($subvalue);
                                    if (!$decoded) {
                                        continue;
                                    }
                                    $mapped = \phpseclib3\File\ASN1::asn1map($decoded[0], $map);
        ...
        if (is_array($extensions)) {
            ...
            for ($i = 0; $i < $size; $i++) {
                        ...
                        for ($j = 0; $j < count($value); $j++) {
                            ...
                            for ($k = 0; $k < count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                if ($map !== \false) {
                                    ...
                                    $subvalue = new Element(\phpseclib3\File\ASN1::encodeDER($subvalue, $map));
                                }
                            }
                        }
                        break;
                    case 'id-ce-authorityKeyIdentifier':
                        ...
                        if (isset($value['authorityCertSerialNumber'])) {
                            if ($value['authorityCertSerialNumber']->toBytes() == '') {
                                $temp = chr(\phpseclib3\File\ASN1::CLASS_CONTEXT_SPECIFIC << 6 | 2) . "\x01\x00";
                ...
                if (is_bool($map)) {
                    ...
                    $value = \phpseclib3\File\ASN1::encodeDER($value, $map, ['iPAddress' => [static::class, 'encodeIP']]);
                }
            }
        }
    }
            ...
            for ($i = 0; $i < count($attributes); $i++) {
                ...
                if (is_array($attributes[$i]['value'])) {
                    ...
                    for ($j = 0; $j < count($values); $j++) {
                        $value = \phpseclib3\File\ASN1::encodeDER($values[$j], Maps\AttributeValue::MAP);
                        $decoded = \phpseclib3\File\ASN1::decodeBER($value);
                        if (!is_bool($map)) {
                            if (!$decoded) {
                                continue;
            ...
            for ($i = 0; $i < $size; $i++) {
                ...
                if ($map === \false) {
                    ...
                    for ($j = 0; $j < count($values); $j++) {
                        ...
                        if (!is_bool($map)) {
                            $temp = \phpseclib3\File\ASN1::encodeDER($values[$j], $map);
                            $decoded = \phpseclib3\File\ASN1::decodeBER($temp);
                            if (!$decoded) {
                                continue;
                            }
                            $values[$j] = \phpseclib3\File\ASN1::asn1map($decoded[0], Maps\AttributeValue::MAP);
                        }
                    }
                }
            }
                ...
                for ($j = 0; $j < count($dns[$i]); $j++) {
                    ...
                    if (is_object($value) && $value instanceof Element) {
                        ...
                        if (!is_bool($map)) {
                            ...
                            $value = \phpseclib3\File\ASN1::asn1map($decoded[0], $map);
                        }
                    }
                }
            }
        ...
        if (is_array($dns)) {
            ...
            for ($i = 0; $i < $size; $i++) {
                for ($j = 0; $j < count($dns[$i]); $j++) {
                    ...
                    $map = $this->getMapping($type);
                    if (!is_bool($map)) {
                        $value = new Element(\phpseclib3\File\ASN1::encodeDER($value, $map));
                    }
                }
            }
        }
    ...
    public function getDNProp($propName, array $dn = null, $withType = \false)
    {
        ...
        $filters = [];
        $filters['value'] = ['type' => \phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        \phpseclib3\File\ASN1::setFilters($filters);
        ...
        for ($i = 0; $i < count($dn); $i++) {
            if ($dn[$i][0]['type'] == $propName) {
                ...
                if (!$withType) {
                    if (is_array($v)) {
                        foreach ($v as $type => $s) {
                            $type = array_search($type, \phpseclib3\File\ASN1::ANY_MAP);
                            if ($type !== \false && array_key_exists($type, \phpseclib3\File\ASN1::STRING_TYPE_SIZE)) {
                                $s = \phpseclib3\File\ASN1::convert($s, $type);
                                if ($s !== \false) {
                                    $v = $s;
                                    break;
                                }
                        ...
                        if (!is_bool($map)) {
                            $decoded = \phpseclib3\File\ASN1::decodeBER($v);
                            if (!$decoded) {
                                return \false;
                            }
                            $v = \phpseclib3\File\ASN1::asn1map($decoded[0], $map);
                        }
                    }
                }
                $result[] = $v;
    ...
    public function getDN($format = self::DN_ARRAY, array $dn = null)
    {
        ...
        switch ((int) $format) {
            ...
            case self::DN_ASN1:
                $filters = [];
                $filters['rdnSequence']['value'] = ['type' => \phpseclib3\File\ASN1::TYPE_UTF8_STRING];
                \phpseclib3\File\ASN1::setFilters($filters);
                $this->mapOutDNs($dn, 'rdnSequence');
                return \phpseclib3\File\ASN1::encodeDER($dn, Maps\Name::MAP);
            case self::DN_CANON:
                ...
                foreach ($dn['rdnSequence'] as $rdn) {
                    foreach ($rdn as $i => $attr) {
                        ...
                        if (is_array($attr['value'])) {
                            foreach ($attr['value'] as $type => $v) {
                                $type = array_search($type, \phpseclib3\File\ASN1::ANY_MAP, \true);
                                if ($type !== \false && array_key_exists($type, \phpseclib3\File\ASN1::STRING_TYPE_SIZE)) {
                                    $v = \phpseclib3\File\ASN1::convert($v, $type);
                                    if ($v !== \false) {
                                        $v = preg_replace('/\\s+/', ' ', $v);
                    ...
                    $result .= \phpseclib3\File\ASN1::encodeDER($rdn, Maps\RelativeDistinguishedName::MAP);
                }
                return $result;
            case self::DN_HASH:
    ...
    public function getChain()
    {
        ...
        foreach ($chain as $key => $value) {
            $chain[$key] = new \phpseclib3\File\X509();
    ...
    public function loadCSR($csr, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcsr = \phpseclib3\File\ASN1::extractBER($csr);
        ...
        $decoded = \phpseclib3\File\ASN1::decodeBER($csr);
        if (!$decoded) {
            $this->currentCert = \false;
            return \false;
        }
        $csr = \phpseclib3\File\ASN1::asn1map($decoded[0], Maps\CertificationRequest::MAP);
    ...
    public function saveCSR(array $csr, $format = self::FORMAT_PEM)
    {
        ...
        $filters = [];
        $filters['certificationRequestInfo']['subject']['rdnSequence']['value'] = ['type' => \phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        \phpseclib3\File\ASN1::setFilters($filters);
        ...
        $csr = \phpseclib3\File\ASN1::encodeDER($csr, Maps\CertificationRequest::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $csr;
            // case self::FORMAT_PEM:
    ...
    public function loadSPKAC($spkac)
    {
        ...
        $decoded = \phpseclib3\File\ASN1::decodeBER($spkac);
        if (!$decoded) {
            $this->currentCert = \false;
            return \false;
        }
        $spkac = \phpseclib3\File\ASN1::asn1map($decoded[0], Maps\SignedPublicKeyAndChallenge::MAP);
    ...
    public function saveSPKAC(array $spkac, $format = self::FORMAT_PEM)
    {
        ...
        $spkac = \phpseclib3\File\ASN1::encodeDER($spkac, Maps\SignedPublicKeyAndChallenge::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $spkac;
            // case self::FORMAT_PEM:
    ...
    public function loadCRL($crl, $mode = self::FORMAT_AUTO_DETECT)
    {
        ...
        if ($mode != self::FORMAT_DER) {
            $newcrl = \phpseclib3\File\ASN1::extractBER($crl);
        ...
        $decoded = \phpseclib3\File\ASN1::decodeBER($crl);
        if (!$decoded) {
            $this->currentCert = \false;
            return \false;
        }
        $crl = \phpseclib3\File\ASN1::asn1map($decoded[0], Maps\CertificateList::MAP);
    ...
    public function saveCRL(array $crl, $format = self::FORMAT_PEM)
    {
        ...
        $filters = [];
        $filters['tbsCertList']['issuer']['rdnSequence']['value'] = ['type' => \phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        $filters['tbsCertList']['signature']['parameters'] = ['type' => \phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        $filters['signatureAlgorithm']['parameters'] = ['type' => \phpseclib3\File\ASN1::TYPE_UTF8_STRING];
        if (empty($crl['tbsCertList']['signature']['parameters'])) {
            $filters['tbsCertList']['signature']['parameters'] = ['type' => \phpseclib3\File\ASN1::TYPE_NULL];
        }
        if (empty($crl['signatureAlgorithm']['parameters'])) {
            $filters['signatureAlgorithm']['parameters'] = ['type' => \phpseclib3\File\ASN1::TYPE_NULL];
        }
        \phpseclib3\File\ASN1::setFilters($filters);
        ...
        $crl = \phpseclib3\File\ASN1::encodeDER($crl, Maps\CertificateList::MAP);
        switch ($format) {
            case self::FORMAT_DER:
                return $crl;
            // case self::FORMAT_PEM:
    ...
    public function setEndDate($date)
    {
        ...
        */
        if (is_string($date) && strtolower($date) === 'lifetime') {
            ...
            $temp = chr(\phpseclib3\File\ASN1::TYPE_GENERALIZED_TIME) . \phpseclib3\File\ASN1::encodeLength(strlen($temp)) . $temp;
    ...
    public function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        switch (\true) {
            ...
            case $key instanceof Element:
                ...
                $decoded = \phpseclib3\File\ASN1::decodeBER($key->element);
                if (!$decoded) {
                    return \false;
                }
                $raw = \phpseclib3\File\ASN1::asn1map($decoded[0], ['type' => \phpseclib3\File\ASN1::TYPE_BIT_STRING]);
                if (empty($raw)) {
                    return \false;
                }
                ...
                // Is a public key.
                break;
            case $key instanceof \phpseclib3\File\X509:
        ...
        $key = \phpseclib3\File\ASN1::extractBER($key);
    ...
    private function formatSubjectPublicKey()
    {
        ...
        $decoded = \phpseclib3\File\ASN1::decodeBER($publicKey);
        if (!$decoded) {
            return \false;
        }
        $mapped = \phpseclib3\File\ASN1::asn1map($decoded[0], Maps\SubjectPublicKeyInfo::MAP);
        if (!is_array($mapped)) {
            return \false;
        }
