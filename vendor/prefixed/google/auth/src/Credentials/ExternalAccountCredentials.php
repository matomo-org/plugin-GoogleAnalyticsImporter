...
use Matomo\Dependencies\GoogleAnalyticsImporter\Google\Auth\CredentialSource\AwsNativeSource;
use Matomo\Dependencies\GoogleAnalyticsImporter\Google\Auth\CredentialSource\ExecutableSource;
use Matomo\Dependencies\GoogleAnalyticsImporter\Google\Auth\CredentialSource\FileSource;
use Matomo\Dependencies\GoogleAnalyticsImporter\Google\Auth\CredentialSource\UrlSource;
use Matomo\Dependencies\GoogleAnalyticsImporter\Google\Auth\ExecutableHandler\ExecutableHandler;
use Matomo\Dependencies\GoogleAnalyticsImporter\Google\Auth\ExternalAccountCredentialSourceInterface;
...
class ExternalAccountCredentials implements FetchAuthTokenInterface, UpdateMetadataInterface, GetQuotaProjectInterface, GetUniverseDomainInterface, ProjectIdProviderInterface
{
    ...
    private $projectId;
    /** @var array<mixed> */
    private $lastImpersonatedAccessToken;
    /**
     * @var string
     */
    ...
    private static function getServiceAccountImpersonationEmail(string $serviceAccountImpersonationUrl) : ?string
    {
     ...
     */
    private function buildRegionalAccessBoundaryLookupUrl(?string $clientName) : string
    {
        if (null !== $clientName) {
            return $this->traitBuildRegionalAccessBoundaryLookupUrl($clientName);
        }
        ...
        if (preg_match($regex, $this->auth->getAudience(), $matches)) {
            ...
            return $this->traitBuildRegionalAccessBoundaryLookupUrl(null, $poolId, $projectNumber);
        }
        ...
        if (preg_match('/locations\\/[^\\/]+\\/workforcePools\\/([^\\/]+)/', $this->auth->getAudience(), $matches)) {
            return $this->traitBuildRegionalAccessBoundaryLookupUrl(null, $matches[1]);
        }
