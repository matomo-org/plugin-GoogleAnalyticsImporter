...
class ImportStatusTest extends IntegrationTestCase
{
    ...
    public function test_startingImport_recordsTheSiteCreationDateAsTheImportRangeStart()
    {
        Date::$now = Date::factory('2015-03-04 00:00:00')->getTimestamp();
        $idSite = Fixture::createWebsite('2012-10-05 00:00:00');
        $this->instance->startingImport('properties/1234', 'account', '', $idSite, [], 'ga4');
        $status = $this->instance->getImportStatus($idSite);
        // both importers create the site from the GA property's create time, so this is the oldest day to import.
        // Fixture::createWebsite() backdates ts_created by a day; addSite() stores the given date as-is.
        $this->assertEquals('2012-10-04', $status['import_range_start']);
    }
    public function test_startingImport_leavesTheImportRangeStartUnsetWhenTheSiteDoesNotExist()
    {
        Date::$now = Date::factory('2015-03-04 00:00:00')->getTimestamp();
        $idSite = 5;
        $this->instance->startingImport('properties/1234', 'account', '', $idSite, [], 'ga4');
        $status = $this->instance->getImportStatus($idSite);
        $this->assertNull($status['import_range_start']);
    }
    public function test_setImportDateRange_keepsTheExistingRangeStartWhenNoStartDateIsGiven()
    {
        Date::$now = Date::factory('2015-03-04 00:00:00')->getTimestamp();
        $idSite = Fixture::createWebsite('2012-10-05 00:00:00');
        $this->instance->startingImport('properties/1234', 'account', '', $idSite, [], 'ga4');
        // Controller::startImportGA4() calls this with a null start whenever only an end date was given;
        // clearing the range start there would undo what startingImport() recorded
        $this->instance->setImportDateRange($idSite, null, Date::factory('2015-03-04'));
        $status = $this->instance->getImportStatus($idSite);
        $this->assertEquals('2012-10-04', $status['import_range_start']);
        $this->assertEquals('2015-03-04', $status['import_range_end']);
    }
