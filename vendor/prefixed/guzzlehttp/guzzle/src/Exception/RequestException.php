...
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\BodySummarizer;
use Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\BodySummarizerInterface;
 ...
 */
class RequestException extends \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\TransferException implements RequestExceptionInterface
{
    /**
     ...
     */
    public static function wrapException(RequestInterface $request, \Throwable $e) : \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\RequestException
    {
        return $e instanceof \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\RequestException ? $e : new \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\RequestException($e->getMessage(), $request, null, $e);
    }
    /**
    ...
    public static function create(RequestInterface $request, ResponseInterface $response = null, \Throwable $previous = null, array $handlerContext = [], BodySummarizerInterface $bodySummarizer = null) : self
    {
        ...
        if ($level === 4) {
            $label = 'Client error';
            $className = \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\ClientException::class;
        } elseif ($level === 5) {
            $label = 'Server error';
            $className = \Matomo\Dependencies\GoogleAnalyticsImporter\GuzzleHttp\Exception\ServerException::class;
        } else {
