...
trait MessageTrait
{
    ...
    public function withProtocolVersion($version) : MessageInterface
    {
        if (!\is_string($version)) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s to MessageInterface::withProtocolVersion() is deprecated; guzzlehttp/psr7 3.0 requires string.', \get_debug_type($version));
        }
    ...
    public function withHeader($header, $value) : MessageInterface
    {
        ...
        foreach ($values as $item) {
            if (!\is_string($item) && (\is_scalar($item) || $item === null)) {
                \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s to MessageInterface::withHeader() is deprecated; guzzlehttp/psr7 3.0 requires string|string[].', \get_debug_type($item));
                break;
            }
        }
    ...
    public function withAddedHeader($header, $value) : MessageInterface
    {
        ...
        foreach ($values as $item) {
            if (!\is_string($item) && (\is_scalar($item) || $item === null)) {
                \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s to MessageInterface::withAddedHeader() is deprecated; guzzlehttp/psr7 3.0 requires string|string[].', \get_debug_type($item));
                break;
            }
        }
        $value = $this->normalizeHeaderValue($value);
    ...
    private function setHeaders(array $headers) : void
    {
        ...
        foreach ($headers as $header => $value) {
            ...
            foreach ($values as $item) {
                if (!\is_string($item) && (\is_scalar($item) || $item === null)) {
                    \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing %s to %s::__construct() is deprecated; guzzlehttp/psr7 3.0 requires string|string[].', \get_debug_type($item), static::class);
                    break;
                }
            }
            $value = $this->normalizeHeaderValue($value);
    ...
    private function normalizeHeaderValue($value) : array
    {
        if (is_array($value) && $value === []) {
            \Matomo\Dependencies\GoogleAnalyticsImporter\trigger_deprecation('guzzlehttp/psr7', '2.11', 'Passing an empty array as a header value is deprecated; guzzlehttp/psr7 3.0 rejects empty header value arrays.');
        }
        if (!is_array($value)) {
