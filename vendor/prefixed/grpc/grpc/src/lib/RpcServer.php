 ...
 * @package Grpc
 */
class RpcServer extends \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\Server
{
    ...
    public function run()
    {
        ...
        while (\true) {
            try {
                ...
                $context = new \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\ServerContext($event);
                $server_writer = new \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\ServerCallWriter($event->call, $context);
                if (!\array_key_exists($full_path, $this->paths_map)) {
                    $context->setStatus(\Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\Status::unimplemented());
                    $server_writer->finish();
                    continue;
                }
                $method_desc = $this->paths_map[$full_path];
                $server_reader = new \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\ServerCallReader($event->call, $method_desc->request_type);
                try {
                    $this->processCall($method_desc, $server_reader, $server_writer, $context);
                } catch (\Exception $e) {
                    $context->setStatus(\Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\Status::status(STATUS_INTERNAL, $e->getMessage()));
                    $server_writer->finish();
                }
            } catch (\Exception $e) {
    ...
    private function processCall(\Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\MethodDescriptor $method_desc, \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\ServerCallReader $server_reader, \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\ServerCallWriter $server_writer, \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\ServerContext $context)
    {
        // Dispatch to actual server logic
        switch ($method_desc->call_type) {
            case \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\MethodDescriptor::UNARY_CALL:
                ...
                break;
            case \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\MethodDescriptor::SERVER_STREAMING_CALL:
                $request = $server_reader->read();
                ...
                break;
            case \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\MethodDescriptor::CLIENT_STREAMING_CALL:
                $response = \call_user_func(array($method_desc->service, $method_desc->method_name), $server_reader, $context);
                $server_writer->finish($response);
                break;
            case \Matomo\Dependencies\GoogleAnalyticsImporter\Grpc\MethodDescriptor::BIDI_STREAMING_CALL:
