...
abstract class DH extends AsymmetricKey
{
    ...
    public static function computeSecret($private, $public)
    {
        ...
        if ($private instanceof \phpseclib3\Crypt\EC\PrivateKey) {
            switch (\true) {
                case $public instanceof \phpseclib3\Crypt\EC\PublicKey:
    ...
    {
        $type = \phpseclib3\Crypt\DH::validatePlugin('Keys', 'PKCS1', 'saveParameters');
        $key = $type::saveParameters($this->prime, $this->base);
        return \phpseclib3\Crypt\DH::load($key, 'PKCS1');
    }
}
