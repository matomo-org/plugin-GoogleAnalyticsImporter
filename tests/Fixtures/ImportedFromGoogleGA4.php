...
class ImportedFromGoogleGA4 extends Fixture
{
    ...
    public function setUp() : void
    {
        ...
        if (SystemTestCase::isCIEnvironment()) {
            $mockResponses = new \Piwik\Plugins\GoogleAnalyticsImporter\tests\Fixtures\MockApiResponsesGA4($createSite = \false);
            $mockResponses->setUp();
        }
    ...
    private function runGoogleImporter($dates, $idSiteToResume = null)
    {
        ...
        Option::set(AuthorizationGA4::ACCESS_TOKEN_OPTION_NAME, $this->accessToken);
        Option::set(AuthorizationGA4::CLIENT_CONFIG_OPTION_NAME, $this->clientConfig);
        $command = "php " . PIWIK_INCLUDE_PATH . '/tests/PHPUnit/proxy/console ' . $domainParam . ' googleanalyticsimporter:import-ga4-reports ';
        if ($idSiteToResume) {
    ...
    public function provideContainerConfig()
    {
        ...
        if (SystemTestCase::isCIEnvironment()) {
            MockResponseClientGA4::$isForSystemTest = \true;
    ...
    private function tryToUseNonTestEnvCredentials()
    {
        ...
        try {
            ...
            $accessToken = Db::fetchOne("SELECT option_value FROM `{$tablesPrefix}option` WHERE option_name = ?", [AuthorizationGA4::ACCESS_TOKEN_OPTION_NAME]);
            if (empty($accessToken)) {
                throw new \Exception("test access token not present as environment variable and not in INI config");
            }
            $this->accessToken = $accessToken;
            $clientConfig = Db::fetchOne("SELECT option_value FROM `{$tablesPrefix}option` WHERE option_name = ?", [AuthorizationGA4::CLIENT_CONFIG_OPTION_NAME]);
